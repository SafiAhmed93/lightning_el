...
6. **Docker:** A `Dockerfile` is provided based on `mambaforge` to handle complex C++ installations (like `turbodbc` and `pyarrow`) on systems like ARM64. Ensure any new native dependencies are added to the `mamba install` step rather than `pip`.
