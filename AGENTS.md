...
6. **Docker:** A `Dockerfile` is provided based on `mambaforge` to handle complex C++ installations. **CRITICAL:** The Dockerfile is strictly locked to `--platform=linux/amd64` because SAP does not provide an ARM64 binary for the HANA ODBC driver. It will run natively on Windows/Linux and via Rosetta 2 emulation on Mac. Always append `--platform=linux/amd64` to any `docker run` or `docker build` commands generated. Ensure any new native dependencies are added to the `mamba install` step rather than `pip`.
